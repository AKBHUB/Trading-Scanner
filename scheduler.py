...
from store.config_store import get_config, resolve_fundamentals_universe
...
def _fundamentals_watchlist():
    """Tier 3's candidate universe — see resolve_fundamentals_universe()
    for the fundamentals_watchlist -> watchlist -> index_universe cascade."""
    return resolve_fundamentals_universe()


def job_news_sentiment():
    """
    Wide open, deliberately not scoped to _watchlist(): news_sentiment.run()
    with no symbols pulls Alpha Vantage's general market feed and writes a
    row per (article, ticker) for every ticker AV tags — broader catalyst
    coverage than the 3-stock watchlist, at the same one-call cost. This
    still covers watchlist symbols whenever AV's general feed mentions them
    (near-certain for large caps like the current AAPL/MSFT/NVDA list), but
    isn't a guarantee the way a ticker-scoped call was — a thinly-covered
    watchlist symbol could go a cycle with no news row. Tier 2/4 and the
    scanner skills are unaffected; they still read _watchlist() as before.
    """
...
def job_fundamentals_watch():
    """
    Runs once daily now (see config/schedule.yaml tiers.fundamentals.check_time)
    instead of on news_sentiment's 30-min cadence, so the M&A lookback window
    widens from 1 hour to 25 hours — enough to cover a full day between
    checks plus a buffer for scheduler drift, without re-flagging the same
    article on two consecutive daily runs.
    """
    watchlist = _fundamentals_watchlist()
    fundamentals.check_earnings_calendar_trigger(watchlist)
    fundamentals.check_news_ma_trigger(watchlist, since=datetime.utcnow() - timedelta(hours=25))
...
def build_scheduler() -> BackgroundScheduler:
    ...
    scheduler.add_job(job_fundamentals_watch, _cron_at(fundamentals_cfg["check_time"]))

    weekly_cfg = fundamentals_cfg.get("weekly_refresh")
    if weekly_cfg:
