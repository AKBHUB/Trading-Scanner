...
from store.config_store import get_config, resolve_fundamentals_universe, set_config
...
with tab_config:
    ...
    st.divider()
    st.subheader("Tier 2 large-volume filter")
    st.caption(
        "Flat share-count floors the sentiment agent applies before scoring — "
        "not relative to average daily volume or float. Congress trades aren't "
        "filterable this way (disclosures only give a dollar range, no share "
        "count)."
    )
    v1, v2 = st.columns(2)
    insider_floor = v1.number_input(
        "Insider transaction floor (shares)",
        min_value=0,
        value=int(get_config("insider_large_shares_threshold", 10_000)),
        step=1000,
    )
    institutional_floor = v2.number_input(
        "Institutional holding floor (shares)",
        min_value=0,
        value=int(get_config("institutional_large_shares_threshold", 100_000)),
        step=10000,
    )
    if st.button("Save volume thresholds"):
        set_config("insider_large_shares_threshold", insider_floor)
        set_config("institutional_large_shares_threshold", institutional_floor)
        st.success("Saved — takes effect on the sentiment agent's next run, no restart needed.")
...
with tab_run:
    ...
    if c3.button("Run fundamentals watch"):
        fund_universe = resolve_fundamentals_universe()
        with st.spinner("Checking earnings calendar and M&A news triggers..."):
            fundamentals.check_earnings_calendar_trigger(fund_universe)
            fundamentals.check_news_ma_trigger(fund_universe, since=datetime.utcnow() - timedelta(hours=25))
...
with tab_fundamentals:
    ...
    st.divider()
    st.subheader("SPX/NDX fallback universe")
    st.caption(
        "Third fallback rung, used only when both the fundamentals ticker "
        "list above and the main watchlist are empty. This is a static list "
        "you maintain by hand here — nothing auto-fetches index membership, "
        "so update it yourself as SPX/NDX constituents change."
    )
    index_universe = get_config("index_universe", [])
    index_universe_text = st.text_area(
        "Symbols (comma-separated)", value=", ".join(index_universe), height=100, key="index_universe_text"
    )
    if st.button("Save SPX/NDX fallback universe"):
        set_config("index_universe", [s.strip().upper() for s in index_universe_text.split(",") if s.strip()])
        st.success("Saved — takes effect next time both higher-priority lists are empty.")

    effective_fund_watchlist = resolve_fundamentals_universe()
    st.caption(
        f"Effective list right now: {', '.join(effective_fund_watchlist) or '(empty — set one of the three lists above)'}"
    )

    st.divider()
    if st.button("Quick refresh", type="primary"):
        if not effective_fund_watchlist:
            st.warning("No tickers to refresh — set a fundamentals, main watchlist, or fallback universe first.")
        else:
